...
class FunctionNode:
    ...
    def __radd__(self, lhs):
        return (lambda *args, **kwargs:
                self.__class__(lhs)(*args, **kwargs) +
                self.__class__(self)(*args, **kwargs))
...
def _pipe_kwarg_bind(*key_positions):
    ...
    def decorator(core_callable):
        ...
        def wrapped(*args, **kwargs):
            ...
            target_kwargs = {k:v for k, v in kwargs.items() if k not in PIPE_NODE_KWARGS}
    ...
    def build_factory(core_callable):
        ...
        def factory_f(*f_args, **f_kwargs):
            ...
            if set(f_kwargs).intersection(PIPE_NODE_KWARGS) != set():
                raise ValueError(f'Either you put a factory in a pipeline (i.e. not a pipe node), or your factory was given a reserved pipeline kwarg {tuple(PIPE_NODE_KWARGS)}.')
            ...
            def expression_f(**e_kwargs):
                ...
                def process_f(*p_args, **p_kwargs):
                    ...
                    return decorated_core_callable(*core_callable_args, **core_callable_kwargs)

                assert set(e_kwargs).intersection(set(f_kwargs)) == set() # This is in impossible state
...
def classmethod_pipe_node_factory(*key_positions, core_decorator=core_logger):
    return _descriptor_factory(*key_positions,
            decorator=pipe_node_factory, core_decorator=core_decorator)


def classmethod_pipe_node(*key_positions, core_decorator=core_logger):
    return _descriptor_factory(*key_positions,
            decorator=pipe_node, core_decorator=core_decorator)
